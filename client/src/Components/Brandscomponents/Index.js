    ...
    if(props.match.params.brand === "brand"){
      return (
        <>
          ...
          <Title title={props.location.search.replace('?','')} subtitle="latest fashion" />
          <div style={{ height: "100px" }}></div>
          <div className="container">
          <div className="row">
            {props.listitems.data
              .filter((person) => person.brand === props.location.search.replace('?',''))
              .map((val, index) => {
                return (
      ...
      const datainput = props.listitems.data.filter((person) => person.quantity === Number(props.location.search.replace('?','')))
      ...
      return (
        <>
          ...
          <div className="container">
          <div className="row">
            {datainput.length === 0 &&  <div className="col-md-12">
                    <h3 style={{textAlign:"center",color:"#1111115c"}}>NO DATA..</h3>
                  </div>
            ...
            <p><input
             type="text"
              className="input_search form-control"
               placeholder="Search.."
               onChange={(e) => setSearchitem(e.target.value)} /> <SearchIcon /></p>
                <hr />
              </div>
              {props.listitems &&
            props.listitems.data
